...
func createIndexes(ctx context.Context, db *bun.DB) error {
	indexes := []struct {
		...
		{name: "idx_vms_user_id", query: "CREATE INDEX IF NOT EXISTS idx_vms_user_id ON vms (user_id)"},
		{name: "idx_vms_user_challenge", query: "CREATE UNIQUE INDEX IF NOT EXISTS idx_vms_user_challenge ON vms (user_id, challenge_id)"},
		{name: "idx_vms_vm_id", query: "CREATE UNIQUE INDEX IF NOT EXISTS idx_vms_vm_id ON vms (vm_id)"},
